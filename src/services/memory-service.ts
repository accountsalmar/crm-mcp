...
import { MemorySession, MemoryMessage, MemoryMetadata } from '../types.js';
...
export async function saveSession(
  ...
  const embeddingTexts = session.messages.map((msg: MemoryMessage, idx: number) =>
  ...
  const records = session.messages.map((msg: MemoryMessage, idx: number) => ({
