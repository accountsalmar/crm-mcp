...
**Returns:** count, total revenue, avg probability per stage, plus optional top opportunities. With \`include_weighted=true\`: weighted revenue per stage + forecast summary (expected/best/worst case).`,
    ...
    // ============================================
    server.registerTool('odoo_crm_get_pipeline_analysis', {
...
- Combine with date filters for time-bound analysis`,
        ...
        try {
            ...
            const activeTotals = await client.readGroup('crm.lead', domain, ['expected_revenue:sum', 'probability:avg', 'id:count'], []);
            const totalActive = activeTotals[0]?.id || 0;
            const totalPipelineRevenue = activeTotals[0]?.expected_revenue || 0;
            const avgProbability = activeTotals[0]?.probability || 0;
            ...
            if (params.group_by === 'salesperson') {
                ...
                analysis.by_salesperson = byUser.map(u => ({
                    ...
                    count: u.id || 0,
                    percentage: totalActive > 0 ? (u.id / totalActive) * 100 : 0,
                    pipeline_revenue: u.expected_revenue || 0,
            ...
            if (params.group_by === 'team') {
                ...
                analysis.by_team = byTeam.map(t => ({
                    ...
                    count: t.id || 0,
                    percentage: totalActive > 0 ? (t.id / totalActive) * 100 : 0,
                    pipeline_revenue: t.expected_revenue || 0,
            ...
            if (params.group_by === 'stage') {
                ...
                analysis.by_stage = byStage.map(s => ({
                    ...
                    count: s.id || 0,
                    percentage: totalActive > 0 ? (s.id / totalActive) * 100 : 0,
                    pipeline_revenue: s.expected_revenue || 0,
                    avg_deal: s.id > 0 ? (s.expected_revenue || 0) / s.id : 0
                })).sort((a, b) => b.count - a.count);
            }
            if (params.group_by === 'month') {
                const byMonth = await client.readGroup('crm.lead', domain, ['expected_revenue:sum', 'id:count'], ['create_date:month']);
                analysis.by_month = byMonth.map(m => ({
                    month: m['create_date:month'] || 'Unknown',
                    count: m.id || 0,
                    pipeline_revenue: m.expected_revenue || 0
                }));
            }
            ...
            if (params.group_by === 'city') {
                ...
                analysis.by_city = byCity.map(c => ({
                    ...
                    count: c.id || 0,
                    percentage: totalActive > 0 ? (c.id / totalActive) * 100 : 0,
                    pipeline_revenue: c.expected_revenue || 0,
                    avg_deal: c.id > 0 ? (c.expected_revenue || 0) / c.id : 0
                })).sort((a, b) => b.count - a.count);
            }
            if (params.group_by === 'architect') {
                const byArchitect = await client.readGroup('crm.lead', domain, ['architect_id', 'expected_revenue:sum', 'id:count'], ['architect_id']);
                analysis.by_architect = byArchitect.map(a => ({
                    architect_id: Array.isArray(a.architect_id) ? a.architect_id[0] : 0,
                    architect_name: Array.isArray(a.architect_id) ? a.architect_id[1] : 'Not Specified',
                    count: a.id || 0,
                    percentage: totalActive > 0 ? (a.id / totalActive) * 100 : 0,
                    pipeline_revenue: a.expected_revenue || 0,
                    avg_deal: a.id > 0 ? (a.expected_revenue || 0) / a.id : 0
                })).sort((a, b) => b.count - a.count);
            }
            if (params.group_by === 'building_owner') {
                const byBuildingOwner = await client.readGroup('crm.lead', domain, ['x_studio_building_owner', 'expected_revenue:sum', 'id:count'], ['x_studio_building_owner']);
                analysis.by_building_owner = byBuildingOwner.map(b => ({
                    ...
                    building_owner_name: Array.isArray(b.x_studio_building_owner) ? b.x_studio_building_owner[1] : 'Not Specified',
                    count: b.id || 0,
                    percentage: totalActive > 0 ? (b.id / totalActive) * 100 : 0,
                    pipeline_revenue: b.expected_revenue || 0,
                    avg_deal: b.id > 0 ? (b.expected_revenue || 0) / b.id : 0
                })).sort((a, b) => b.count - a.count);
            }
            // Get top active opportunities
            if (params.include_top_opportunities > 0) {
                const topOpps = await client.searchRead('crm.lead', domain, ['id', 'name', 'expected_revenue', 'probability', 'user_id', 'stage_id'], { limit: params.include_top_opportunities, order: 'expected_revenue desc' });
        ...
        try {
            ...
            if (params.group_by === 'architect') {
                ...
                analysis.by_architect = byArchitect.map(a => ({
                    ...
                    architect_name: Array.isArray(a.architect_id) ? a.architect_id[1] : 'Not Specified',
                    count: a.id || 0,
                    percentage: totalLost > 0 ? (a.id / totalLost) * 100 : 0,
                    lost_revenue: a.expected_revenue || 0,
                    avg_deal: a.id > 0 ? (a.expected_revenue || 0) / a.id : 0
                })).sort((a, b) => b.count - a.count);
            }
            if (params.group_by === 'building_owner') {
                ...
                analysis.by_building_owner = byBuildingOwner.map(b => ({
                    ...
                    building_owner_name: Array.isArray(b.x_studio_building_owner) ? b.x_studio_building_owner[1] : 'Not Specified',
                    count: b.id || 0,
                    percentage: totalLost > 0 ? (b.id / totalLost) * 100 : 0,
                    lost_revenue: b.expected_revenue || 0,
                    avg_deal: b.id > 0 ? (b.expected_revenue || 0) / b.id : 0
                })).sort((a, b) => b.count - a.count);
            }
        ...
        try {
            ...
            if (params.group_by === 'architect') {
                ...
                analysis.by_architect = byArchitect.map(a => ({
                    ...
                    architect_name: Array.isArray(a.architect_id) ? a.architect_id[1] : 'Not Specified',
                    count: a.id || 0,
                    percentage: totalWon > 0 ? (a.id / totalWon) * 100 : 0,
                    won_revenue: a.expected_revenue || 0,
                    avg_deal: a.id > 0 ? (a.expected_revenue || 0) / a.id : 0
                })).sort((a, b) => b.count - a.count);
            }
            if (params.group_by === 'building_owner') {
                ...
                analysis.by_building_owner = byBuildingOwner.map(b => ({
                    ...
                    building_owner_name: Array.isArray(b.x_studio_building_owner) ? b.x_studio_building_owner[1] : 'Not Specified',
                    count: b.id || 0,
                    percentage: totalWon > 0 ? (b.id / totalWon) * 100 : 0,
                    won_revenue: b.expected_revenue || 0,
                    avg_deal: b.id > 0 ? (b.expected_revenue || 0) / b.id : 0
                })).sort((a, b) => b.count - a.count);
            }
