 ...
 * @returns Formatted string
 */
export function formatSemanticSearchResults(matches, leads, query, format) {
    ...
    for (let i = 0; i < matches.length; i++) {
        ...
        if (lead || meta) {
            ...
            const statusBadges = [];
            if (meta?.is_won)
                statusBadges.push('✅ Won');
            if (meta?.is_lost)
                statusBadges.push('❌ Lost');
            if (!meta?.is_won && !meta?.is_lost)
            ...
            const roles = [];
            if (meta?.architect_name)
                roles.push(`Architect: ${meta.architect_name}`);
            if (meta?.project_manager_name)
                roles.push(`PM: ${meta.project_manager_name}`);
            if (meta?.estimator_name)
                roles.push(`Estimator: ${meta.estimator_name}`);
            if (meta?.spec_rep_name)
                roles.push(`Spec Rep: ${meta.spec_rep_name}`);
            if (roles.length > 0) {
 ...
 * @returns Formatted string
 */
export function formatSimilarDeals(matches, leads, reference, format) {
    // Build a map of lead ID to lead data for fast lookup
    const leadMap = new Map();
    for (const lead of leads) {
        leadMap.set(lead.id, lead);
    ...
    if (reference.priority_label)
        output += ` | Priority: ${reference.priority_label}`;
    output += '\n';
    ...
    if (reference.sector || reference.specification_name) {
        let classLine = '- ';
        if (reference.sector)
            classLine += `Sector: ${reference.sector}`;
        if (reference.sector && reference.specification_name)
            classLine += ' | ';
        if (reference.specification_name)
            classLine += `Spec: ${reference.specification_name}`;
        output += classLine + '\n';
    }
    const refLocation = [reference.city, reference.state_name].filter(Boolean).join(', ');
    if (refLocation)
        output += `- Location: ${refLocation}\n`;
    output += '\n';
    ...
    const formatDealGroup = (deals, title, emoji) => {
        if (deals.length === 0)
            return '';
 ...
 * @returns Formatted string
 */
export function formatPatternDiscovery(result, format) {
    // JSON format
    if (format === ResponseFormat.JSON) {
        return JSON.stringify(result, null, 2);
    }
        ...
        if (cluster.representativeDeals.length > 0) {
            ...
            for (const deal of cluster.representativeDeals) {
                ...
                if (deal.expected_revenue)
                    details.push(`Revenue: ${formatCurrency(deal.expected_revenue)}`);
                if (deal.stage_name)
                    details.push(`Stage: ${deal.stage_name}`);
                if (details.length > 0) {
                    output += `  ${details.join(' | ')}\n`;
                }
                // Company/Partner
 ...
 * @returns Formatted string
 */
export function formatVectorStatus(status) {
 ...
 */
function formatCircuitBreakerState(state) {
    switch (state) {
        case 'CLOSED':
 ...
 */
export function formatMemorySession(messages, format) {
    if (format === ResponseFormat.JSON) {
 ...
 */
export function formatMemorySearch(results, query, format) {
    if (format === ResponseFormat.JSON) {
 ...
 */
export function formatSessionList(sessions, format) {
    if (format === ResponseFormat.JSON) {
 ...
 */
export function formatMemoryStatus(health, format) {
    if (format === ResponseFormat.JSON) {
